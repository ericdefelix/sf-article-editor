...
let editor = {
  ...
  _bindEvtSelectionDropdown: function() {
    ...
    const blockquoteIcon = blockquote.querySelector('.blockquote-addon');

    blockquote.className = 'blockquote';
    blockquote.classList.add('blockquote-' + selectedStyle);

    for (let i = 0; i <= ContentBlocks.elems['blockQuotes'].types.length - 1; i++) {
      const type = ContentBlocks.elems['blockQuotes'].types[i];

      if (type.ui_value === selectedStyle) {
        blockquoteIcon.innerHTML = type.svg;
        break;
      }
    }
  },
